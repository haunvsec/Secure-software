...
def vendors_list():
    ...
    if search:
        result = get_vendors(session, search=search, page=page)
    ...
    if not letter:
        letter = 'A'
    result = get_vendors(session, letter=letter, page=page)
    return render_template('vendors.html', active_page='vendors',
                           vendors=result['items'], pagination=result,
...
def vendor_detail(vendor):
    session = get_session()
    vd = get_vendor_detail(session, vendor)
    if vd is None:
        return render_template('404.html', active_page=''), 404
    page = sanitize_page(request.args.get('page'))
    result = get_vendor_products(session, vendor, page)
